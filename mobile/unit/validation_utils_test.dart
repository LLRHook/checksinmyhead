  ...
  group('ValidationUtils tests', () {
    group('isValidPhoneNumber tests', () {
      ...
      test('rejects invalid phone numbers', () {
        expect(ValidationUtils.isValidPhoneNumber('123456'), false); // too short
        expect(ValidationUtils.isValidPhoneNumber('12345678901234'), false); // too long
        expect(ValidationUtils.isValidPhoneNumber('21234567890'), false); // 11 digits but wrong country code
        expect(ValidationUtils.isValidPhoneNumber('abcdefghij'), false); // not numeric
      });
    });
    ...
    group('isValidEmail tests', () {
      ...
      test('rejects invalid email addresses', () {
        ...
        expect(ValidationUtils.isValidEmail('@example.com'), false); // no username
        expect(ValidationUtils.isValidEmail('test@example'), false); // incomplete domain
        expect(ValidationUtils.isValidEmail('test@.com'), false); // missing domain name
        expect(ValidationUtils.isValidEmail('test@example.'), false); // incomplete TLD
      });
    });
    ...
    group('isValidVenmoUsername tests', () {
      test('validates valid Venmo usernames', () {
        ...
        expect(ValidationUtils.isValidVenmoUsername('@username_with_underscore'), true);
      });
      
      test('rejects invalid Venmo usernames', () {
        expect(ValidationUtils.isValidVenmoUsername('user'), false); // missing @ prefix
        expect(ValidationUtils.isValidVenmoUsername('@'), false); // no username after @
    ...
    group('isValidCashtag tests', () {
      ...
      test('rejects invalid Cashtags', () {
        expect(ValidationUtils.isValidCashtag('user'), false); // missing $ prefix
        expect(ValidationUtils.isValidCashtag('\$'), false); // no username after $
