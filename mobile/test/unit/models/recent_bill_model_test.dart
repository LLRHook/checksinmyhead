...
void main() {
  group('RecentBillModel.generatePersonShares', () {
    ...
    test('returns empty map when no participants and no items', () {
      final bill = RecentBillModel.fromData(
        _makeRecentBill(participants: [], total: 30.0),
      );
    ...
    test('handles single participant', () {
      final bill = RecentBillModel.fromData(
        _makeRecentBill(participants: ['Alice'], total: 50.0),
      );
    ...
    test('calculates shares from item assignments', () {
      ...
      final aliceShare =
          shares.entries.firstWhere((e) => e.key.name == 'Alice').value;
      final bobShare =
          shares.entries.firstWhere((e) => e.key.name == 'Bob').value;
  ...
  group('RecentBillModel.participantSummary', () {
    test('shows "No participants" when empty', () {
      final bill = RecentBillModel.fromData(_makeRecentBill(participants: []));
      expect(bill.participantSummary, equals('No participants'));
    });

    test('joins two names with ampersand', () {
      final bill = RecentBillModel.fromData(
        _makeRecentBill(participants: ['Alice', 'Bob']),
      );
    ...
    test('shows count for 3+ participants', () {
      final bill = RecentBillModel.fromData(
        _makeRecentBill(participants: ['Alice', 'Bob', 'Charlie', 'Dana']),
      );
  ...
  group('RecentBillModel.fromData', () {
    test(
      'retains original currency and derives a stable rounded USD total',
      () {
        final bill = RecentBillModel.fromData(
          _makeRecentBill(
            total: 19.99,
    ...
    test('parses tip percentage from stored value', () {
      final bill = RecentBillModel.fromData(
        _makeRecentBill(tipPercentage: 18.0),
      );
    ...
    test('calculates tip percentage when not stored', () {
      final bill = RecentBillModel.fromData(
        _makeRecentBill(subtotal: 100.0, tipAmount: 20.0, tipPercentage: null),
      );
    ...
    test('handles zero subtotal when calculating tip percentage', () {
      final bill = RecentBillModel.fromData(
        _makeRecentBill(subtotal: 0.0, tipAmount: 0.0, tipPercentage: null),
      );
