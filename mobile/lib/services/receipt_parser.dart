...
class ReceiptParser {
  ...
  /// Price at end of line, allowing an optional trailing tax-flag letter
  /// (A, B, F, T, N, O, X) and/or asterisk — common on grocery receipts.
  static final _priceAtEnd = RegExp(
    r'\$?\s*(\d+\.\d{2})\s*[A-Za-z]?\s*\*?\s*$',
  );

  /// Standalone price-only line (just a dollar amount, nothing else).
  static final _standalonePriceOnly = RegExp(
    r'^\s*\$?\s*\d+\.\d{2}\s*[A-Za-z]?\s*\*?\s*$',
  );
