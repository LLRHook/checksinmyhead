...
class ParticipantsProvider extends ChangeNotifier {
  ...
  void addPerson(String name, {Color? color}) {
    ...
    if (_participants.any((p) => p.name.toLowerCase() == name.trim().toLowerCase())) {
      return;
    }
  ...
  bool addRecentPerson(Person person) {
    if (_participants.any((p) => p.name.toLowerCase() == person.name.toLowerCase())) {
  ...
  bool isPersonSelected(Person person) {
    return _participants.any((p) => p.name.toLowerCase() == person.name.toLowerCase());
  }
  ...
  void toggleRecentPerson(Person person) {
    ...
    if (isSelected) {
      _participants.removeWhere((p) => p.name.toLowerCase() == person.name.toLowerCase());
      HapticFeedback.selectionClick();
    } else {
